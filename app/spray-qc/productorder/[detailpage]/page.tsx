...
export default function ProductOrderDetailPage({ params }: DetailPageProps) {
  ...
  const isCurrentStepOneValid = sprayState.spId && sprayState.rejectedQuantity;
      ...
      <div className="flex flex-col gap-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-[#171717]">SP ID <span className="text-red-500">*</span></label>
            <select value={sprayState.spId} onChange={(e) => setSprayState({ ...sprayState, spId: e.target.value })} className={`h-[42px] rounded-[8px] border ${showValidationHint && !sprayState.spId ? "border-red-500 bg-red-50" : "border-[#DDE1E8]"} px-3 text-[14px] bg-white`}>
              <option value="">Select SP ID</option>
              <option value="SP-0001">SP-0001</option>
              <option value="SP-0002">SP-0002</option>
              <option value="SP-0003">SP-0003</option>
            </select>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-[#171717]">Rejected Quantity <span className="text-red-500">*</span></label>
            <input type="number" value={sprayState.rejectedQuantity} onChange={(e) => setSprayState({ ...sprayState, rejectedQuantity: e.target.value })} className={`h-[42px] rounded-[8px] border ${showValidationHint && !sprayState.rejectedQuantity ? "border-red-500 bg-red-50" : "border-[#DDE1E8]"} px-3 text-[14px]`} />
          </div>
        </div>
      </div>
    );
  ...
  const renderReviewCards = () => {
    const fields = [
      ...
      { label: "Rejected Quantity", value: sprayState.rejectedQuantity },
    ];
    return (
