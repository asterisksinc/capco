  ...
  const getBreadcrumbs = () => {
    ...
    if (adminIndex !== -1) {
      ...
      for (let i = 0; i < relevantSegments.length; i++) {
        ...
        let name = "Overview";
        let href = `/admin/${relevantSegments.slice(0, i + 1).join("/")}`;
        
        if (segment === "workorders") name = "Work Orders";
        else if (segment === "productorders") name = "Product Orders";
        else if (segment === "inventory") name = "Inventory";
        else if (segment === "pipeline") name = "Pipeline";
        else if (segment === "users") name = "User Management";
        else if (segment === "crm") name = "CRM";
        else if (segment === "invoices") name = "Invoices";
        else if (segment === "finishedgoods") name = "Finished Goods";
    ...
    if (paths.length === 0) {
      paths.push({ name: "Overview", href: "/admin/overview" });
    }
